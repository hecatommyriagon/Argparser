 ...
 * @brief Implementations for Argparser
 * @version 1.0
 *
...
void Argparser::add_option (char opt, bool args_allowed, bool required, int max = -1, int min = -1) {
    ...
    if (min > max || max <= 0 || min < 0) {
        ...
        "Invalid argument min/max argument settings for flag" + std::string (1, opt));
    }
...
void Argparser::parse () {
    ...
    for (int i = 1; i < argv.size (); i++) {
        ...
        if (argv.at (i)[0] == '-') {
            ...
            for (int j = 0; j < options.size (); j++) {
                if (argv.at (i) == "-" + std::string (1, options.at (j).flag)) {
                    ...
                    // end loop since we found a match, and skip iterating
                    // through the next few args if they applied to this flag
                    i += skip;
                    break;
                }
            }
        ...
        if (valid_arg == false) {
            throw std::invalid_argument ("Invalid argument " + argv.at (i));
        }
    }
    verify ();
}
    ...
    for (Option option : options) {
        ...
        if (option.required == true) {
            ...
            if (option.on == false) {
                ...
                std::string (1, option.flag) + " is required but not used");
                return false;
            }
        }
        ...
        // if this flag does not meet min or max argument values
            ...
            if (option.values.size () < option.min) {
                ...
                throw std::runtime_error ("Flag -" +
                std::string (1, option.flag) + " does not have enough arguments");
                return false;
            }
            ...
            if (option.values.size () > option.max) {
                ...
                throw std::runtime_error (
                "Flag -" + std::string (1, option.flag) + " as too many arguments");
                return false;
            }
        }
    }
...
std::vector<int> Argparser::get_values_int (char flag) {
    ...
    throw std::runtime_error (
    "Option -" + std::string (1, flag) + " is not a user-specified option");
}
